...
fn grammar<J: Jet>() -> Grammar<Ast<J>> {
    ...
    santiago::grammar!(
        ...
        "expr" => rules "HIDDEN" "symbol" => |mut toks| {
            if let Some(e) = propagate_errors(&toks) { return e; }
            assert_eq!(toks.len(), 2);
            let position = toks[0].expect_dummy();
            let cmr = match toks[1].parse_cmr() {
                Ok(cmr) => cmr,
                Err(e) => return e,
            };
            Ast::Expression(Expression {
                variant: ExpressionVariant::Hidden(cmr),
                position,
            })
        };
        ...
        "HIDDEN" => lexemes "HIDDEN" => Ast::from_lexeme;
