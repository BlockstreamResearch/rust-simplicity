...
static bool applyBinding_cont(unification_var* alpha, binding* bound, unification_cont** cont, size_t* bindings_used) {
  ...
  if (&alpha->bound == bound) {
    rustsimplicity_0_7_assert(false); /* The algorithm should never try to bind an already bound variable to it's own binding. */
  ...
  if (ONE == bound->kind) {
    ...
    *cont = (*cont)->next;
    ...
    rustsimplicity_0_7_assert(0 < *bindings_used);
    (*bindings_used)--;
  }
  return true;
}
...
static bool isFrozen(unification_var* var) {
  rustsimplicity_0_7_assert(!var->isBound || ONE != var->bound.kind || 0 == var->bound.frozen_ix);
...
static bool freeze(size_t* result, type* type_dag, size_t* type_dag_used, unification_var* var) {
   ...
   */
  var->next = NULL;
  rustsimplicity_0_7_assert(!var->bound.occursCheck);
...
static simplicity_err freezeTypes(type* type_dag, dag_node* dag, unification_arrow* arrow, const size_t len) {
  ...
  rustsimplicity_0_7_computeTypeAnalyses(type_dag, type_dag_used);
 ...
 */
simplicity_err rustsimplicity_0_7_mallocTypeInference(type** type_dag, rustsimplicity_0_7_callback_mallocBoundVars mallocBoundVars, dag_node* dag, const uint_fast32_t len, const combinator_counters* census) {
  *type_dag = NULL;
  ...
  unification_arrow* arrow = rustsimplicity_0_7_malloc(len * sizeof(unification_arrow));
  ...
  rustsimplicity_0_7_assert(orig_bindings_used <= NUMBER_OF_TYPENAMES_MAX - 1);
  ...
  if (IS_OK(result)) {
    ...
    rustsimplicity_0_7_assert(bindings_used <= orig_bindings_used + 4*len);
    *type_dag = rustsimplicity_0_7_malloc((1 + bindings_used) * sizeof(type));
  ...
  rustsimplicity_0_7_free(arrow);
  rustsimplicity_0_7_free(bound_var);
  return result;
}
