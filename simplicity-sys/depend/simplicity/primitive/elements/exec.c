 ...
 */
extern bool rustsimplicity_0_4_elements_execSimplicity( simplicity_err* error, unsigned char* ihr
  ...
  rustsimplicity_0_4_assert(NULL != tx);
  rustsimplicity_0_4_assert(NULL != taproot);
  rustsimplicity_0_4_assert(NULL != genesisBlockHash);
  rustsimplicity_0_4_assert(0 <= budget);
  rustsimplicity_0_4_assert(NULL != program || 0 == program_len);
  rustsimplicity_0_4_assert(NULL != witness || 0 == witness_len);
...

  {
    ...
    dag_len = rustsimplicity_0_4_decodeMallocDag(&dag, &census, &stream);
    if (dag_len <= 0) {
  ...
  if (IS_OK(*error)) {
    type* type_dag = NULL;
    *error = rustsimplicity_0_4_mallocTypeInference(&type_dag, dag, (uint_fast32_t)dag_len, &census);
    if (IS_OK(*error)) {
      rustsimplicity_0_4_assert(NULL != type_dag);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_4_fillWitnessData(dag, type_dag, (uint_fast32_t)dag_len, &witness_stream);
      if (IS_OK(*error)) {
        *error = rustsimplicity_0_4_closeBitstream(&witness_stream);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_4_verifyNoDuplicateIdentityHashes(&ihr_buf, dag, type_dag, (uint_fast32_t)dag_len);
    ...
    if (IS_OK(*error) && amr) {
      ...
      analyses *analysis = rustsimplicity_0_4_malloc((size_t)dag_len * sizeof(analyses));
      if (analysis) {
        rustsimplicity_0_4_computeAnnotatedMerkleRoot(analysis, dag, type_dag, (uint_fast32_t)dag_len);
      ...
      rustsimplicity_0_4_free(analysis);
    }
    if (IS_OK(*error)) {
      txEnv env = rustsimplicity_0_4_build_txEnv(tx, taproot, &genesis_hash, ix);
