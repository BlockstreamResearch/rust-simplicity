...
static inline bool writeBit(frameItem* frame, bool bit) {
  rustsimplicity_0_8_debug_assert(0 < frame->offset);
  frame->offset--;
 ...
 * Precondition: '*frame' is a valid read frame for 'N' more cells.
 */
uint_fast8_t rustsimplicity_0_8_read4(frameItem* frame);
uint_fast8_t rustsimplicity_0_8_read8(frameItem* frame);
uint_fast16_t rustsimplicity_0_8_read16(frameItem* frame);
uint_fast32_t rustsimplicity_0_8_read32(frameItem* frame);
uint_fast64_t rustsimplicity_0_8_read64(frameItem* frame);
 ...
 * Precondition: '*frame' is a valid write frame for 'N' more cells.
 */
void rustsimplicity_0_8_write8(frameItem* frame, uint_fast8_t x);
void rustsimplicity_0_8_write16(frameItem* frame, uint_fast16_t x);
void rustsimplicity_0_8_write32(frameItem* frame, uint_fast32_t x);
void rustsimplicity_0_8_write64(frameItem* frame, uint_fast64_t x);

static inline void read8s(unsigned char* x, size_t n, frameItem* frame) {
  for(; n; --n) *(x++) = (unsigned char)rustsimplicity_0_8_read8(frame);
}

static inline void write8s(frameItem* frame, const unsigned char* x, size_t n) {
  for(; n; --n) rustsimplicity_0_8_write8(frame, *(x++));
}

static inline void read32s(uint32_t* x, size_t n, frameItem* frame) {
  for(; n; --n) *(x++) = (uint32_t)rustsimplicity_0_8_read32(frame);
}

static inline void write32s(frameItem* frame, const uint32_t* x, size_t n) {
  for(; n; --n) rustsimplicity_0_8_write32(frame, *(x++));
}
 ...
 *               0 <= n < 16
 */
void rustsimplicity_0_8_read_buffer8(unsigned char* buf, size_t* len, frameItem* src, int n);
 ...
 *               0 <= n < 16;
 */
void rustsimplicity_0_8_write_buffer8(frameItem* dst, const unsigned char* buf, size_t len, int n);
 ...
 */
bool rustsimplicity_0_8_read_sha256_context(sha256_context* ctx, frameItem* src);
 ...
 *               ctx->counter < 2^61;
 */
bool rustsimplicity_0_8_write_sha256_context(frameItem* dst, const sha256_context* ctx);
 ...
 */
void rustsimplicity_0_8_copyBits(frameItem* dst, const frameItem* src, size_t n);
#endif
