...
static sha256_midstate mkJetCMR(uint32_t *ih, uint_fast64_t weight) {
  ...
  rustsimplicity_0_8_sha256_compression(result.s, block);
 ...
 */
sha256_midstate rustsimplicity_0_8_computeWordCMR(const bitstring* value, size_t n) {
  /* 'stack' is an array of 30 hashes consisting of 8 'uint32_t's each. */
  uint32_t stack[8*30] = {0};
  uint32_t *stack_ptr = stack;
  sha256_midstate ih = identityIV;
  rustsimplicity_0_8_assert(n < 32);
  rustsimplicity_0_8_assert((size_t)1 << n == value->len);
 ...
 *               dag[i].'tag' \notin {HIDDEN, JET, WORD}
 */
void rustsimplicity_0_8_computeCommitmentMerkleRoot(dag_node* dag, const uint_fast32_t i) {
  uint32_t block[16] = {0};
  size_t j = 8;

  rustsimplicity_0_8_assert(HIDDEN != dag[i].tag);
  rustsimplicity_0_8_assert(JET != dag[i].tag);
  rustsimplicity_0_8_assert(WORD != dag[i].tag);
  ...
  switch (dag[i].tag) {
   ...
   case DROP:
    ...
    rustsimplicity_0_8_sha256_compression(dag[i].cmr.s, block);
    SIMPLICITY_FALLTHROUGH;
   case IDEN:
   case UNIT:
   case WITNESS:
...
static void computeIdentityHashRoots(sha256_midstate* ihr, const dag_node* dag, const type* type_dag, const uint_fast32_t len) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    switch (dag[i].tag) {
     case WITNESS:
      rustsimplicity_0_8_sha256_bitstring(block, &dag[i].compactValue);
      memcpy(block + 8, type_dag[WITNESS_B(dag, type_dag, i)].typeMerkleRoot.s, sizeof(uint32_t[8]));
      rustsimplicity_0_8_sha256_compression(ihr[i].s, block);
      break;
     case COMP:
     case ASSERTL:
     case ASSERTR:
 ...
 */
void rustsimplicity_0_8_computeAnnotatedMerkleRoot(analyses* analysis, const dag_node* dag, const type* type_dag, const uint_fast32_t len) {
  for (uint_fast32_t i = 0; i < len; ++i) {
    ...
    switch (dag[i].tag) {
     ...
     case CASE:
      ...
      rustsimplicity_0_8_sha256_compression(analysis[i].annotatedMerkleRoot.s, block);
     ...
     case WITNESS:
      ...
      rustsimplicity_0_8_sha256_bitstring(block + 8, &dag[i].compactValue);
      rustsimplicity_0_8_sha256_compression(analysis[i].annotatedMerkleRoot.s, block);
      break;
     case HIDDEN:
     case JET:
 ...
 */
simplicity_err rustsimplicity_0_8_verifyCanonicalOrder(dag_node* dag, const uint_fast32_t len) {
  uint_fast32_t bottom = 0;
  uint_fast32_t top = len-1; /* Underflow is checked below. */

  if (!len) {
    rustsimplicity_0_8_assert(false); /* A well-formed dag has non-zero length */
  ...
  rustsimplicity_0_8_assert(bottom == top && top == len);
 ...
 */
simplicity_err rustsimplicity_0_8_fillWitnessData(dag_node* dag, type* type_dag, const uint_fast32_t len, bitstream *witness) {
      ...
      if (witness->len <= 0) {
        ...
        while (cur) {
          if (SUM == type_dag[cur].kind) {
            ...
            rustsimplicity_0_8_assert(calling);
            ...
            rustsimplicity_0_8_assert(PRODUCT == type_dag[cur].kind);
            size_t next;
            if (calling) {
 ...
 */
simplicity_err rustsimplicity_0_8_verifyNoDuplicateIdentityHashes(sha256_midstate* ihr, const dag_node* dag, const type* type_dag, const uint_fast32_t dag_len) {
  rustsimplicity_0_8_assert(0 < dag_len);
  rustsimplicity_0_8_assert(dag_len <= DAG_LEN_MAX);
  sha256_midstate* ih_buf = rustsimplicity_0_8_malloc((size_t)dag_len * sizeof(sha256_midstate));
  ...
  int result = rustsimplicity_0_8_hasDuplicates(ih_buf, dag_len);

  rustsimplicity_0_8_free(ih_buf);
