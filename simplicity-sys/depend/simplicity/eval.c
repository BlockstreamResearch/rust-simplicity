...
static void backward(frameItem* frame, size_t n) {
  rustsimplicity_0_7_debug_assert(n <= frame->offset);
  frame->offset -= n;
}
...
static void writeValue(frameItem* dst, const bitstring* compactValue, size_t typeIx, type* type_dag) {
  ...
  while (cur) {
    if (SUM == type_dag[cur].kind) {
      ...
      rustsimplicity_0_7_debug_assert(PRODUCT == type_dag[cur].kind);
      size_t next;
      if (calling) {
...
/* 'calling' lets us know if we are entering a CALL or returning from a CALL. */
  ...
  while(pc < len) {
    ...
    rustsimplicity_0_7_debug_assert(state.activeReadFrame < state.activeWriteFrame);
    rustsimplicity_0_7_debug_assert(state.activeReadFrame->edge <= state.activeWriteFrame->edge);
    if (dag[pc].jet) {
    ...
    switch (tag) {
     case COMP:
      if (calling) {
        ...
        *(state.activeWriteFrame - 1) = initWriteFrame(type_dag[COMP_B(dag, type_dag, pc)].bitSize, state.activeWriteFrame->edge);
        ...
        /* MOVE_FRAME */
        rustsimplicity_0_7_debug_assert(0 == state.activeWriteFrame->offset);
     ...
     case DISCONNECT:
      if (calling) {
        ...
        *(state.activeWriteFrame - 1) = initWriteFrame(type_dag[DISCONNECT_W256A(dag, type_dag, pc)].bitSize,
        ...
        /* COPY(BITSIZE(A)) */
        rustsimplicity_0_7_copyBits(state.activeWriteFrame, state.activeReadFrame, type_dag[DISCONNECT_A(dag, type_dag, pc)].bitSize);
        ...
        *(state.activeReadFrame + 1) = initReadFrame(type_dag[DISCONNECT_BC(dag, type_dag, pc)].bitSize,
        ...
        /* COPY(BITSIZE(B)) */
        rustsimplicity_0_7_copyBits(state.activeWriteFrame, state.activeReadFrame, type_dag[DISCONNECT_B(dag, type_dag, pc)].bitSize);
     ...
     case WITNESS:
      if (IDEN == tag) {
        /* COPY(BITSIZE(A)) */
        rustsimplicity_0_7_copyBits(state.activeWriteFrame, state.activeReadFrame, type_dag[IDEN_A(dag, type_dag, pc)].bitSize);
      } else {
  ...
  rustsimplicity_0_7_assert(pc == len);
...
static simplicity_err antiDos(flags_type checks, const call* stack, const dag_node* dag, size_t len) {
  ...
  rustsimplicity_0_7_assert(CHECK_CASE == (checks & CHECK_CASE) || 0 == (checks & CHECK_CASE));
 ...
 */
simplicity_err rustsimplicity_0_7_analyseBounds( ubounded *cellsBound, ubounded *UWORDBound, ubounded *frameBound, ubounded *costBound
  ...
  rustsimplicity_0_7_assert(1 <= len);
  rustsimplicity_0_7_assert(len <= DAG_LEN_MAX);
  boundsAnalysis* bound = rustsimplicity_0_7_malloc(len * sizeof(boundsAnalysis));
 ...
 *               if 'dag[len]' represents a Simplicity expression with primitives then 'NULL != env';
 */
simplicity_err rustsimplicity_0_7_evalTCOExpression( flags_type anti_dos_checks, UWORD* output, const UWORD* input
                                           , const dag_node* dag, type* type_dag, size_t len, ubounded minCost, const ubounded* budget, const txEnv* env
                                           ) {
  rustsimplicity_0_7_assert(1 <= len);
  rustsimplicity_0_7_assert(len <= DAG_LEN_MAX);
  if (budget) {
    rustsimplicity_0_7_assert(*budget <= BUDGET_MAX);
    rustsimplicity_0_7_assert(minCost <= *budget);
  }
  rustsimplicity_0_7_assert(minCost <= BUDGET_MAX);
  ...
  simplicity_err result = rustsimplicity_0_7_analyseBounds(&cellsBound, &UWORDBound, &frameBound, &costBound, CELLS_MAX, minCost*1000, budget ? *budget*1000 : UBOUNDED_MAX, dag, type_dag, len);
  ...
  rustsimplicity_0_7_assert(frameBound <= 2*len);
  ...
  rustsimplicity_0_7_assert(UWORDBound <= (cellsBound + frameBound*(UWORD_BIT - 1)) / UWORD_BIT);

  /* UWORDBound, is also at most the cellsBound, with an entire UWORD per cell (the rest of the UWORD being padding). */
  rustsimplicity_0_7_assert(UWORDBound <= cellsBound);
  ...
  UWORD* cells = rustsimplicity_0_7_calloc(UWORDBound ? UWORDBound : 1, sizeof(UWORD));
  ...
  frameItem* frames = rustsimplicity_0_7_malloc(frameBound * sizeof(frameItem));
  call* stack = rustsimplicity_0_7_calloc(len, sizeof(call));

  result = cells && frames && stack ? SIMPLICITY_NO_ERROR : SIMPLICITY_ERR_MALLOC;
  if (IS_OK(result)) {
    const ubounded inputSize = type_dag[dag[len-1].sourceType].bitSize;
    const ubounded outputSize = type_dag[dag[len-1].targetType].bitSize;
    rustsimplicity_0_7_assert(NULL != input || 0 == inputSize);
    ...
    *(state.activeWriteFrame) = initWriteFrame(outputSize, cells + UWORDBound);
    ...
    if (IS_OK(result)) {
      rustsimplicity_0_7_assert(NULL != output || 0 == outputSize);
  ...
  rustsimplicity_0_7_free(stack);
  rustsimplicity_0_7_free(frames);
  rustsimplicity_0_7_free(cells);
  return result;
}
