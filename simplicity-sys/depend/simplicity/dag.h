...
static inline size_t CASE_A(const dag_node* dag, const type* type_dag, size_t i) {
  rustsimplicity_0_7_debug_assert(CASE == dag[i].tag || ASSERTL == dag[i].tag || ASSERTR == dag[i].tag);
  rustsimplicity_0_7_debug_assert(PRODUCT == type_dag[dag[i].sourceType].kind);
  rustsimplicity_0_7_debug_assert(SUM == type_dag[type_dag[dag[i].sourceType].typeArg[0]].kind);
...
static inline size_t PAIR_A(const dag_node* dag, const type* type_dag, size_t i) {
  (void)type_dag;
  rustsimplicity_0_7_debug_assert(PAIR == dag[i].tag);
  return dag[i].sourceType;
}

static inline size_t PAIR_B(const dag_node* dag, const type* type_dag, size_t i) {
  rustsimplicity_0_7_debug_assert(PAIR == dag[i].tag);
  rustsimplicity_0_7_debug_assert(PRODUCT == type_dag[dag[i].targetType].kind);
...
static inline size_t DISCONNECT_A(const dag_node* dag, const type* type_dag, size_t i) {
  (void)type_dag;
  rustsimplicity_0_7_debug_assert(DISCONNECT == dag[i].tag);
  return dag[i].sourceType;
}
...
static inline size_t INJ_A(const dag_node* dag, const type* type_dag, size_t i) {
  (void)type_dag;
  rustsimplicity_0_7_debug_assert(INJL == dag[i].tag || INJR == dag[i].tag);
  return dag[i].sourceType;
}

static inline size_t INJ_B(const dag_node* dag, const type* type_dag, size_t i) {
  rustsimplicity_0_7_debug_assert(INJL == dag[i].tag || INJR == dag[i].tag);
  rustsimplicity_0_7_debug_assert(SUM == type_dag[dag[i].targetType].kind);
  return type_dag[dag[i].targetType].typeArg[0];
}
...
static inline size_t PROJ_A(const dag_node* dag, const type* type_dag, size_t i) {
  rustsimplicity_0_7_debug_assert(TAKE == dag[i].tag || DROP == dag[i].tag);
  rustsimplicity_0_7_debug_assert(PRODUCT == type_dag[dag[i].sourceType].kind);
...
static inline size_t WITNESS_A(const dag_node* dag, const type* type_dag, size_t i) {
  (void)type_dag;
  rustsimplicity_0_7_debug_assert(WITNESS == dag[i].tag);
  return dag[i].sourceType;
}
 ...
 */
void rustsimplicity_0_7_computeCommitmentMerkleRoot(dag_node* dag, uint_fast32_t i);
 ...
 */
void rustsimplicity_0_7_computeAnnotatedMerkleRoot(analyses* analysis, const dag_node* dag, const type* type_dag, uint_fast32_t len);
