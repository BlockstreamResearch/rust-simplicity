 ...
 */
bool rustsimplicity_0_7_elements_computeCmr( simplicity_err* error, unsigned char* cmr
                                   , const unsigned char* program, size_t program_len) {
  rustsimplicity_0_7_assert(NULL != error);
  rustsimplicity_0_7_assert(NULL != cmr);
  rustsimplicity_0_7_assert(NULL != program || 0 == program_len);

  bitstream stream = initializeBitstream(program, program_len);
  dag_node* dag = NULL;
  int_fast32_t dag_len = rustsimplicity_0_7_decodeMallocDag(&dag, rustsimplicity_0_7_elements_decodeJet, NULL, &stream);
  if (dag_len <= 0) {
