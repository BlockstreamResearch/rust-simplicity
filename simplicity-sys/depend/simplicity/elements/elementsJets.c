...
static void amt(frameItem* dst, const confAmount* amt) {
  if (writeBit(dst, EXPLICIT == amt->prefix)) {
    ...
    rustsimplicity_0_7_write64(dst, amt->explicit);
  } else {
    ...
    if (writeBit(dst, j < env->tx->output[i].pnd.len)) {
      if (writeBit(dst, OP_PUSHBYTES_64 < env->tx->output[i].pnd.op[j].code)) {
        ...
        if (writeBit(dst, OP_1 <= env->tx->output[i].pnd.op[j].code)) {
          ...
          rustsimplicity_0_7_debug_assert(OP_RESERVED == env->tx->output[i].pnd.op[j].code ||
...
bool rustsimplicity_0_7_total_fee(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  rustsimplicity_0_7_write64(dst, lookup_fee(&assetid, env->tx->feeOutputs, env->tx->numFees));
  return true;
}
...
bool rustsimplicity_0_7_broken_do_not_use_tx_lock_distance(frameItem* dst, frameItem src, const txEnv* env) {
  (void) src; // src is unused;
  rustsimplicity_0_7_write16(dst, obsolete_lockDistance(env->tx));
  return true;
}
...
bool rustsimplicity_0_7_broken_do_not_use_tx_lock_duration(frameItem* dst, frameItem src, const txEnv* env) {
  (void) src; // src is unused;
  rustsimplicity_0_7_write16(dst, obsolete_lockDuration(env->tx));
  return true;
}
...
bool rustsimplicity_0_7_calculate_issuance_entropy(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  op.ix = rustsimplicity_0_7_read32(&src);
  read32s(contract.s, 8, &src);

  result = rustsimplicity_0_7_generateIssuanceEntropy(&op, &contract);
...
bool rustsimplicity_0_7_calculate_asset(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  result = rustsimplicity_0_7_calculateAsset(&entropy);
...
bool rustsimplicity_0_7_calculate_explicit_token(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  read32s(entropy.s, 8, &src);
  result = rustsimplicity_0_7_calculateToken(&entropy, EXPLICIT);
...
bool rustsimplicity_0_7_calculate_confidential_token(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  read32s(entropy.s, 8, &src);
  result = rustsimplicity_0_7_calculateToken(&entropy, EVEN_Y /* ODD_Y would also work. */);
...
bool rustsimplicity_0_7_build_tapleaf_simplicity(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  sha256_midstate cmr;
  readHash(&cmr, &src);
  sha256_midstate result = rustsimplicity_0_7_make_tapleaf(0xbe, &cmr);
  writeHash(dst, &result);
  return true;
}
...
bool rustsimplicity_0_7_build_tapbranch(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  sha256_midstate result = rustsimplicity_0_7_make_tapbranch(&a, &b);
  writeHash(dst, &result);
  return true;
}
...
bool rustsimplicity_0_7_output_hash(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  if (writeBit(dst, i < env->tx->numOutputs)) {
    ...
    rustsimplicity_0_7_sha256_confAsset(&ctx, &output->asset);
    rustsimplicity_0_7_sha256_confAmt(&ctx, &output->amt);
    rustsimplicity_0_7_sha256_confNonce(&ctx, &output->nonce);
...
bool rustsimplicity_0_7_input_utxo_hash(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  if (writeBit(dst, i < env->tx->numInputs)) {
    ...
    sha256_midstate midstate;
    sha256_context ctx = sha256_init(midstate.s);
    rustsimplicity_0_7_sha256_confAsset(&ctx, &txo->asset);
    rustsimplicity_0_7_sha256_confAmt(&ctx, &txo->amt);
...
bool rustsimplicity_0_7_issuance_hash(frameItem* dst, frameItem src, const txEnv* env) {
  ...
  if (writeBit(dst, i < env->tx->numInputs)) {
    ...
    if (NO_ISSUANCE == issuance->type) {
      ...
      rustsimplicity_0_7_sha256_confAsset(&ctx, &(confidential){ .prefix = EXPLICIT, .data = issuance->assetId});
      rustsimplicity_0_7_sha256_confAmt(&ctx, &issuance->assetAmt);
      rustsimplicity_0_7_sha256_confAsset(&ctx, &(confidential){ .prefix = EXPLICIT, .data = issuance->tokenId});
      rustsimplicity_0_7_sha256_confAmt(&ctx, NEW_ISSUANCE == issuance->type
                                         ? &issuance->tokenAmt
