 ...
 */
extern bool rustsimplicity_0_8_elements_execSimplicity( simplicity_err* error, unsigned char* ihr
  ...
  rustsimplicity_0_8_assert(NULL != error);
  rustsimplicity_0_8_assert(NULL != tx);
  rustsimplicity_0_8_assert(NULL != taproot);
  rustsimplicity_0_8_assert(NULL != genesisBlockHash);
  rustsimplicity_0_8_assert(0 <= minCost);
  rustsimplicity_0_8_assert(minCost <= budget);
  rustsimplicity_0_8_assert(NULL != program || 0 == program_len);
  rustsimplicity_0_8_assert(NULL != witness || 0 == witness_len);
...

  {
    ...
    dag_len = rustsimplicity_0_8_decodeMallocDag(&dag, rustsimplicity_0_8_elements_decodeJet, &census, &stream);
    if (dag_len <= 0) {
      rustsimplicity_0_8_assert(dag_len < 0);
      *error = (simplicity_err)dag_len;
      return IS_PERMANENT(*error);
    }
    rustsimplicity_0_8_assert(NULL != dag);
    rustsimplicity_0_8_assert((uint_fast32_t)dag_len <= DAG_LEN_MAX);
  ...
  if (IS_OK(*error)) {
    type* type_dag = NULL;
    *error = rustsimplicity_0_8_mallocTypeInference(&type_dag, rustsimplicity_0_8_elements_mallocBoundVars, dag, (uint_fast32_t)dag_len, &census);
    if (IS_OK(*error)) {
      rustsimplicity_0_8_assert(NULL != type_dag);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_8_fillWitnessData(dag, type_dag, (uint_fast32_t)dag_len, &witness_stream);
      if (IS_OK(*error)) {
        *error = rustsimplicity_0_8_closeBitstream(&witness_stream);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_8_verifyNoDuplicateIdentityHashes(&ihr_buf, dag, type_dag, (uint_fast32_t)dag_len);
    ...
    if (IS_OK(*error) && amr) {
      ...
      analyses *analysis = rustsimplicity_0_8_malloc((size_t)dag_len * sizeof(analyses));
      if (analysis) {
        rustsimplicity_0_8_computeAnnotatedMerkleRoot(analysis, dag, type_dag, (uint_fast32_t)dag_len);
    ...
    if (IS_OK(*error)) {
      txEnv env = rustsimplicity_0_8_elements_build_txEnv(tx, taproot, &genesis_hash, ix);
