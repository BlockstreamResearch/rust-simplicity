...
static void sort_buckets(const sha256_midstate** a, uint32_t* restrict bucketSize, const uint32_t* restrict bucketEdge, unsigned int ix) {
  ...
  for (unsigned int i = 0; i < CHAR_COUNT; ++i) {
    ...
    while (bucketSize[i]) {
      ...
      rustsimplicity_0_8_assert(bucketSize[bucket]);
      bucketSize[bucket]--;
...
static void rsort_ex(const sha256_midstate** a, uint_fast32_t len, const sha256_midstate** hasDuplicates, uint32_t* stack) {
  ...
  while(totalBucketCount) {
    ...
    while(0 == bucketCount[depth]) {
      rustsimplicity_0_8_assert(depth);
      depth--;
    }
    ...
    if (2 <= len - stack[totalBucketCount]) {
      ...
      if (depth < sizeof((*a)->s)) {
        ...
        rustsimplicity_0_8_assert(len == bucketEdge[UCHAR_MAX] + bucketSize[UCHAR_MAX]);
 ...
 * Precondition: For all 0 <= i < len, NULL != a[i];
 */
bool rustsimplicity_0_8_rsort(const sha256_midstate** a, uint_fast32_t len) {
  uint32_t *stack = rustsimplicity_0_8_malloc(((CHAR_COUNT - 1)*(sizeof((*a)->s)) + 1) * sizeof(uint32_t));
 ...
 *               len <= DAG_LEN_MAX;
 */
int rustsimplicity_0_8_hasDuplicates(const sha256_midstate* a, uint_fast32_t len) {
  if (len < 2) return 0;
  ...
  rustsimplicity_0_8_assert((size_t)len <= SIZE_MAX / sizeof(const sha256_midstate*));
  const sha256_midstate **perm = rustsimplicity_0_8_malloc(len * sizeof(const sha256_midstate*));
  uint32_t *stack = rustsimplicity_0_8_malloc(((CHAR_COUNT - 1)*(sizeof((*perm)->s)) + 1) * sizeof(uint32_t));
