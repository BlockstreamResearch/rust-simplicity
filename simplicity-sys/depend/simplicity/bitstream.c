 ...
 */
simplicity_err rustsimplicity_0_8_closeBitstream(bitstream* stream) {
 ...
 *               NULL != stream
 */
int32_t rustsimplicity_0_8_readNBits(int n, bitstream* stream) {
  rustsimplicity_0_8_assert(0 <= n && n < 32);
...
static int32_t decodeUpto3Bits(int32_t* result, bitstream* stream) {
  ...
  *result = 0;
  if (0 == bit) {
    ...
    if (0 <= n) {
      *result = rustsimplicity_0_8_readNBits(n, stream);
      if (*result < 0) return *result;
    }
    return n;
  }
 ...
 * Precondition: NULL != stream
 */
int32_t rustsimplicity_0_8_decodeUptoMaxInt(bitstream* stream) {
  int32_t bit = read1Bit(stream);
  if (bit < 0) return bit;
  if (0 == bit) {
    return 1;
    ...
    {
      int32_t result = rustsimplicity_0_8_readNBits(n, stream);
      if (result < 0) return result;
      return ((1 << n) | result);
    }
  }
 ...
 *               NULL != stream
 */
simplicity_err rustsimplicity_0_8_readBitstring(bitstring* result, size_t n, bitstream* stream) {
  static_assert(0x80000000u + 2*(CHAR_BIT - 1) <= SIZE_MAX, "size_t needs to be at least 32-bits");
  rustsimplicity_0_8_assert(n <= 0x80000000u);
