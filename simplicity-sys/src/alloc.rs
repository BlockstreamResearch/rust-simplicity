...
/// Allocated bytes must be freed using [`rust_0_7_free`].
#[no_mangle]
pub unsafe extern "C" fn rust_0_7_malloc(size_bytes: usize) -> *mut u8 {
...
/// # Safety
///
/// - `ptr_bytes` must have been allocated using [`rust_0_7_malloc`] or [`rust_0_7_calloc`].
/// - If `ptr_bytes` is a `NULL` pointer, then this function is a NO-OP.
#[no_mangle]
pub unsafe extern "C" fn rust_0_7_free(ptr_bytes: *mut u8) {
    if ptr_bytes.is_null() {
        return;
    }

    // We got a pointer to an allocation from `rust_0_7_malloc` or `rust_0_7_calloc`,
