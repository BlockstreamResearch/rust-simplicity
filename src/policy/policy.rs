    ...
    <Pk as str::FromStr>::Err: ToString,
{
    fn from_tree(top: &expression::Tree) -> Result<Policy<Pk>, msError> {
        type PolicyError = miniscript::policy::concrete::PolicyError;
        match (top.name, top.args.len() as u32) {
            ...
            ("and", _) => {
                if top.args.len() != 2 {
                    return Err(msError::PolicyError(PolicyError::NonBinaryArgAnd));
                }
            ...
            ("or", _) => {
                if top.args.len() != 2 {
                    return Err(msError::PolicyError(PolicyError::NonBinaryArgOr));
                }
                let mut subs = Vec::with_capacity(top.args.len());
                for arg in &top.args {
                    subs.push(Policy::from_tree(arg)?);
